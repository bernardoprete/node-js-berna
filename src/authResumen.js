/* 
    ...
    ** diferencia entre hasheo y encriptación, es que la primera es un proceso de un unico sentido, en cambio la encriptación puede desencripatarse.
    ** en el hasheo, es el 'salt', que es un conjunto de carácteres especiales que se adjuntan a la contraseña ingresada, generando una cadena unica, esto significa que dos usuario que tengan la misma contraseña, tienen distintos hash.
    ...
    - JWT: Es un estandar compacto y seguro para el intercambio de información entre partes, codificadas como un objeto JSON. Este estandar nos permite generar un 'token' de acceso a travñes del cual vamos a poder identificar si el usuario ingreso al sistema, y ademas si ese ingreso es válido, es decir, no existe una manipulación del mismo por parte del usuario.

    Cuando un usuario ingresa al sistema exitosamente, nuestro servidor Nodejs genera un JWT, este contienen información relevante sobre el usuario (su id, email, rol, etc..) estas propiedades son conocidas como 'claims' en el Payload. Es importante tener en cuenta que el JWT no esta encriptado especificamente para ocultar la información. En su lugar, esta 'firmado' digitalmente con una clave secreta (solo conoce el servidor). Esta firma garantiza dos cosas fundamentales:
    ...
    ej: {
        ...
        "exp": 1231231231233 // especifica cuando expira el token // claim standrs
    }
