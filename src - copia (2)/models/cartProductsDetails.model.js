 ...
 */
    try {
      const columns = Object.keys(cartItem);
      const placeholders = columns.map(() => "?").join(", ");
      const values = Object.values(cartItem);
 ...
 */
  ...
  async findItemsInCart(idCarrito) {
    ...
    try {
      ...
      //Son necesarios los AS abajo? duda  -----------------------------------------
      const sql = `
      SELECT 
      ...
      INNER JOIN producto p ON cp.${this.fields.idProducto} = p.${p.id}
